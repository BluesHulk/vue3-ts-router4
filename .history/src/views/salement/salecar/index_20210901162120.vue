<template>
  <div class="saleCar-management-container">
    <vab-query-form>
      ...
      <vab-query-form>
        <vab-query-form-left-panel :span="24">
          ...
          >
            ...
            <el-form-item label="计划单号:" prop="orderNum">
              <el-input
...
<script>
  ...
  import {
    ...
    querySaleOrderList,
    queryDpOrderList,
    CoSalesDpOrderControllerUpdate,
    receivingOrders,
    receivingRevoke,
    getDpOrderInfoByRid,
    getPlanInfoByRid,
    CoSalesupdateupdateAuditStatus,
    CoSalesupdateSwitchStatus,
    CoSalesupdateDelFlag,
  } from '@/api/sale'
    ...
    methods: {
      ...
      receivingOrders(row) {
        ...
        this.$baseConfirm('确定接单吗', null, async () => {
          const { message } = await receivingOrders({
            rid: row.rid,
          })
          this.$baseMessage(
            message,
      ...
      returnOrder(row) {
        //撤单 接口需要调换
        this.$baseConfirm('确定撤单吗', null, async () => {
          const { message, status, code } = await receivingRevoke({
            rid: row.rid,
          })
      ...
      canclePart() {
        ...
        this.$refs['ticketList'].resetFields()
      },
      ...
      addCofirmOrder() {
        if (this.orderRows.length > 1) {
          ...
          this.multipleSelectionorder.map((val) => {
            if (val.rid) {
              //销售计划详情
              getPlanInfoByRid({ rid: val.rid }).then((res) => {
                const { code, data } = res
                if (code === 200) {
                  this.orderItem = data
                }
      ...
      checkStatus(selectRows, status, falg) {
        ...
        if (auditStatus === status && falg) {
          ...
          if (status == '1' && switchStatus == '1' && state == '1') {
            //处于未审核状态
            CoSalesupdateupdateAuditStatus({
              auditStatus: status,
              rid,
            ...
            if (val === 'del') {
              ...
              if (auditStatus === '1') {
                ...
                this.$baseConfirm('你确定要删除选中项吗', null, async () => {
                  const { message } = await CoSalesupdateDelFlag({
      ...
      resetFormPart() {
        this.$refs['carForm'].resetFields()
        console.log(this.carVal, this.isUnit)
        this.fetchCar(this.carVal, this.isUnit)
      },
      ...
      async fetchOrderList() {
        ...
        const res = await querySaleOrderDpPlanList(this.orderQueryForm)
        if (res.code === 200) {
          const { list, page } = res.data
          this.orderList = list
      ...
      async fetchData() {
        ...
        param.paramList = [
          ...
          },
          {
            mark: 'like',
            name: 'dpPlanCode',
            value: this.queryForm.dpPlanCode,
          },
          {
            mark: 'like',
            name: 'consignerName',
          ...
          },
          {
            mark: 'like',
            name: 'prodName',
            value: this.queryForm.prodName,
          },
          {
            mark: 'like',
            name: 'receiverName',
            value: this.queryForm.receiverName,
          },
          {
            mark: 'like',
            name: 'switchStatus',
          ...
          },
          {
            mark: 'like',
            name: 'driverName',
            value: this.queryForm.driverName,
          },
          {
            mark: 'like',
            name: 'truckId',
            value: this.queryForm.truckId,
          },
          {
            mark: 'like',
            name: 'startTime',
        ...
        // queryDpOrderList
        const res = await queryDpOrderList(param)
        if (res.code === 200) {
          const { list, page } = res.data
          this.list = list
