...
router.beforeEach(async (to, from, next) => {
  ...
  const localTime = store.getters['user/localTime']
  const stepMin = store.getters['user/stepMin']
  var ispast
  ...
  if (hasToken) {
    if (store.getters['routes/routes'].length) {
      ...
      try {
        if (loginInterception && hasToken && to.path !== '/login' && ispast) {
          await store.dispatch('user/getUserInfo')
        } else {
          // config/setting.config.js loginInterception为false(关闭登录拦截时)时，创建虚拟角色
          await store.dispatch('user/setVirtualRoles')
        }
        let index = store.getters['user/idIndex']
        let item = store.getters['user/routerLink'][index] //用户信息有的工厂id
        // 根据路由模式获取路由并根据权限过滤
        ...
        if (to.path === '/module') {
          ...
          //区分一个系统还是多个系统 调用菜单接口
          if (iIndex) {
            await store.dispatch('routes/setRoutes', { id: iIndex })
          } else {
            await store.dispatch('routes/setRoutes', { id: item.rid })
          }
