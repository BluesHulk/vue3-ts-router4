...
const globalStoreActions: ActionTree<globalStore, any> = {
  ...
  async concatRoutes({ commit }, routers) {
    ...
    // routerList.map((respon) => {
    //   router.addRoute(respon);
    // })
    // console.log(routerList)
    // commit("CONCAT_ROUTES", [...routerList, ...initialRoutes]);

    // setRouter(JSON.stringify([...routerList, ...initialRoutes]))
    // return [...routerList, ...initialRoutes]

    let routerList = [];
    const { data, code, status } = await getMenuTreeBySystemCode()

    let initialRoutes = router.options.routes
    routerList = recursionRouter(data, asyncRouter);
    routerList.map((respon: RouteRecordRaw) => {
      router.addRoute(respon);
    })
    console.log(routerList)
    commit("CONCAT_ROUTES", [...routerList, ...initialRoutes]);
