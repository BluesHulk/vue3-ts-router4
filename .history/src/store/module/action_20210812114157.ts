...
const globalStoreActions: ActionTree<globalStore, any> = {
  ...
  async setRoutes({ commit }, options = {}) {
    // 默认前端路由
    let routes = [...asyncRoutes]
    // 设置后端路由(不需要可以删除)
    const iId = options.id
    if (authentication === 'all') {
      // const { list } = await getRouterList(options)
      if (iId) {
        const res = await getMenuTreeBySystemCode()
        ...
        if (code === 200 && status === 0) {
          if (!isArray(data)) {
            ...
            const fliter = filterAsyncRouter(asyncRoutes, data)
            fliter.push({ path: '*', redirect: '/404', hidden: true })
            // debugger
            routes = convertRouter(fliter)
          }
        } else {
          if (iId === 1 && !isArray(data)) {
            router.push({ path: '/module' }).then(() => {
              // Vue.prototype.$baseMessage(
              //   '路由格式返回有误！',
              //   'error',
              //   false,
