...
import {
  recursionRouter,
  setDefaultRoute,
} from "../../utils/recursion-router.js";
...
const globalStoreActions: ActionTree<globalStore, any> = {
  ...
  setlhTenantCode({ commit }, lhTenantCode) {
    commit("setlhTenantCode", lhTenantCode);
  },
  ...
  async login({ commit }, userInfo) {
    ...
    if (data.code == 200 && data.status == 0) {
      ...
      commit("setlhTenantCode", data.data.user.tenantCode);
  ...
  async concatRoutes({ commit }, routers) {
    ...
    const routerList = recursionRouter(data, asyncRouter);
